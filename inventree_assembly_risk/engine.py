...
def audit_totals(stock_inventory, production_demand) -> dict:
    """Return concise, serializable totals for Packing List / diagnostics.

    This helper deliberately reports both gross StockItem quantity and the
    quantities which actually participate in Assembly Risk. It prevents a
    packing-list "Available Qty" value from being mistaken for free stock.

    ``stock_inventory`` is the diagnostic inventory generated by plugin.py.
    ``production_demand`` is the competing Production BuildLine debug list.
    """
    stock_inventory = list(stock_inventory or [])
    production_demand = list(production_demand or [])

    gross_physical_quantity = sum(
        (max(dec(row.get("quantity", 0)), Decimal("0")) for row in stock_inventory),
        Decimal("0"),
    )
    existing_allocations = sum(
        (max(dec(row.get("allocated", 0)), Decimal("0")) for row in stock_inventory),
        Decimal("0"),
    )
    free_before_location_exclusions = sum(
        (max(dec(row.get("raw_free", 0)), Decimal("0")) for row in stock_inventory),
        Decimal("0"),
    )
    excluded_free_stock = sum(
        (
            max(dec(row.get("raw_free", 0)), Decimal("0"))
            for row in stock_inventory
            if row.get("excluded_by_location")
        ),
        Decimal("0"),
    )
    usable_free_before_demand = sum(
        (max(dec(row.get("usable_free", 0)), Decimal("0")) for row in stock_inventory),
        Decimal("0"),
    )
    outstanding_production_demand = sum(
        (
            max(
                dec(
                    row.get(
                        "outstanding_before_virtual_allocation",
                        row.get("gross_requirement", 0),
                    )
                ),
                Decimal("0"),
            )
            for row in production_demand
        ),
        Decimal("0"),
    )

    return {
        "gross_physical_quantity": gross_physical_quantity,
        "existing_allocations": existing_allocations,
        "free_before_location_exclusions": free_before_location_exclusions,
        "excluded_free_stock": excluded_free_stock,
        "usable_free_before_demand": usable_free_before_demand,
        "outstanding_production_demand": outstanding_production_demand,
    }
...
def location_is_excluded(path_parts: Iterable[str], extra_names: Iterable[str] = ()) -> bool:
    ...
    names = DEFAULT_EXCLUDED_LOCATION_NAMES | {
        str(x).strip().lower() for x in extra_names if str(x).strip()
    }
    values = [str(x or "").strip() for x in path_parts]

    # A StockItem with no location is still physical stock. Only explicitly
    # excluded locations (including any Rework ancestor) are ignored.
    if not any(values):
        return False
