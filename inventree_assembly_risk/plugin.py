...
class AssemblyRiskPlugin(SettingsMixin, UserInterfaceMixin, InvenTreePlugin):
    ...
    SETTINGS = {
        ...
        "CACHE_SECONDS": {
            ...
            "description": _(
                "Seconds to reuse an Assembly Risk production snapshot. A short "
                "cache prevents the dashboard and Build Order panel from repeating "
                "the same expensive calculation back-to-back. Set to 0 to disable "
                "caching."
            ),
            "default": 30,
            "validator": int,
    ...
    def _production_status_value():
        """Return the DB value for BuildStatus.PRODUCTION across releases."""
    ...
    def _is_production_build(cls, build) -> bool:
        try:
            return (
                int(getattr(build, "status", -1))
                == cls._production_status_value()
            )
        except Exception:
            return False
    ...
    def _location_path_parts(location):
        ...
        try:
            return [
                str(x.name)
                for x in location.get_ancestors(include_self=True)
            ]
        except Exception:
            return [str(getattr(location, "name", ""))]

    def _stock_inventory_snapshot(self, relevant_part_ids):
        """Return usable stock plus diagnostics for each physical StockItem."""
        relevant_part_ids = {
            int(x) for x in relevant_part_ids if x is not None
        }
        ...
        for item in items:
            ...
            allocated = max(
                dec(allocated_by_item.get(item.pk, 0)),
                Decimal("0"),
            )
    ...
    def _candidate_part_ids(part, allow_variants: bool):
        ...
        try:
            root = part.get_root()
            ids.update(
                root.get_descendants(include_self=True).values_list(
                    "pk",
                    flat=True,
                )
            )
            ...
            try:
                ids.update(
                    part.get_descendants(include_self=True).values_list(
                        "pk",
                        flat=True,
                    )
                )
    ...
    # ---------- Production demand snapshot ----------

    def _production_requirements(self, included_build_ids=None):
        """Build allocation-aware demand for selected Production Build Orders.

        ``included_build_ids=None`` preserves legacy behavior and includes all
        Production Build Orders.

        For an explicit scope, only the listed Production BOs contribute future
        demand. Stock already allocated to unselected BOs still remains unavailable
        because physical free stock is calculated globally.
        """
        ...
        for line in lines:
            ...
            bom_remaining = max(
                dec(getattr(line, "quantity", 0))
                - dec(getattr(line, "consumed", 0)),
                Decimal("0"),
            )
            ...
            allow_variants = bool(
                getattr(bom, "allow_variants", False)
            )
            ...
            if meta_key not in component_meta:
                ...
                component_meta[meta_key] = {
                    "part_id": part.pk,
                    "part_name": (
                        getattr(part, "IPN", None)
                        or part.name
                    ),
                    "part_full_name": getattr(
                        part,
                        "full_name",
                        str(part),
                    ),
                    "allow_variants": allow_variants,
                    "candidate_ids": self._candidate_part_ids(
                        part,
                        allow_variants,
                    ),
                    "spillage": spillage,
            ...
            for allocation in line.allocations.all():
                qty = max(
                    dec(getattr(allocation, "quantity", 0)),
                    Decimal("0"),
                )
                ...
                allocation_details.append(
                    {
                        ...
                        "stock_item_id": getattr(
                            allocation,
                            "stock_item_id",
                            None,
                        ),
                        "stock_part_id": getattr(
                            stock_item,
                            "part_id",
                            None,
                        ),
            ...
            gross_requirement = (
                unallocated_bom + applied_spillage
            )
    ...
    def _allocate_free_stock_items(requirements, stock_items):
        """Virtually satisfy outstanding demand from remaining free stock."""
        remaining = {
            stock_id: dec(data.get("free", 0))
    ...
    def _calculate_uncached(self, included_build_ids=None):
        """Run one allocation-aware Production Demand Snapshot."""
        ...
        for req in requirements:
            relevant_part_ids.update(
                req["candidate_ids"]
            )

        stock_items, stock_inventory_debug = (
            self._stock_inventory_snapshot(
                relevant_part_ids
            )
        )

        self._allocate_free_stock_items(
            requirements,
            stock_items,
        )
        ...
        for req in requirements:
            shortage_by_part[req["part_id"]] += req.get(
                "unfilled",
                Decimal("0"),
            )
            builds_by_part[req["part_id"]].add(
                req["build_ref"]
            )
        ...
        for req in requirements:
            compatible_items = [
                ...
                if data.get("part_id")
                in req["candidate_ids"]
            ]

            req["physical_buffer"] = sum(
                dec(data.get("final_free", 0))
                for data in compatible_items
            )

            req["global_shortage"] = (
                shortage_by_part[req["part_id"]]
            )
            req["builds"] = sorted(
                builds_by_part[req["part_id"]]
            )
    ...
    def _calculate(self, included_build_ids=None):
        """Return a cached Production Assembly Risk snapshot.

        The cache key includes the selected BO set, preventing one packing-list
        scope from reusing another scope's result.
        """
        included_build_ids = normalize_build_ids(
            included_build_ids
        )
        ...
        cache_key = (
            f"inventree-assembly-risk:{self.VERSION}:"
            f"production-snapshot:"
    ...
    def production_demand_snapshot(
        self,
        included_build_ids=None,
    ):
        """Public reusable Production-demand snapshot.

        None means all Production BOs. An explicit iterable selects the BOs whose
        future demand should be included.
        """
        return self._calculate(
            included_build_ids=included_build_ids
        )

    def assembly_risk_for_stock_item(
        self,
        build_id,
        stock_item_id,
        included_build_ids=None,
    ):
        """Return Assembly Risk for one BO / StockItem pair.

        Scope:
        - included_build_ids=None -> all Production BOs
        - explicit iterable -> only selected Production BO demand

        For explicit scopes the queried Build Order is always included.
        """
        try:
        ...
        included_build_ids = include_queried_build(
            build_id,
            included_build_ids,
        )
        ...
        for req in calculated:
                ...
                demand_debug.append(
                    {
                        ...
                        "existing_allocated_to_line": other[
                            "allocated"
                        ],
                        ...
                        "planned_spillage_applied": other[
                            "spillage"
                        ],
                        "spillage_rule": other[
                            "spillage_rule"
                        ],
                        "pricing_max": other.get(
                            "pricing_max",
                            Decimal("0"),
                        ),
                        "pricing_source": other.get(
                            "pricing_source",
                            "",
                        ),
                        "gross_requirement": other[
                            "gross_requirement"
                        ],
                        "outstanding_before_virtual_allocation": (
                            other["required"]
                        ),
                        "existing_allocations": other.get(
                            "existing_allocations",
                            [],
                        ),
                        "virtual_allocations": other.get(
                            "virtual_allocations",
                            [],
                        ),
                        "unfilled": other.get(
            ...
            return {
                ...
                "allocated_to_build_line": req[
                    "allocated"
                ],
                "outstanding": req["required"],
                "planned_spillage": req[
                    "spillage"
                ],
                ...
                "stock_item_quantity": item.get(
                    "quantity"
                ),
                "stock_item_allocated_total": item.get(
                    "allocated"
                ),
                "stock_item_free_before_simulation": item.get(
                    "free"
                ),
                "stock_item_final_free": item.get(
                    "final_free"
                ),
                ...
                "scope": {
                    ...
                    "included_build_refs": scope_build_refs,
                },
                "debug": {
                    "queried_build_id": build_id,
                    "queried_stock_item_id": (
                        stock_item_id
                    ),
                    "stock_inventory": req.get(
                        "stock_inventory_debug",
                        [],
                    ),
    ...
    def _should_show_global_requirement(req):
        ...
        physical_buffer = dec(
            req.get("physical_buffer", 0)
        )

        return (
            spillage > 0
            and physical_buffer <= 0
        )

    def _rows_for_build(self, build_id):
        rows = []

        # Existing Build Order panel intentionally retains the global Production
        # view. Packing List is the feature which supplies an explicit scope.
        for req in self._calculate():
            ...
            if other_builds:
                message += (
                    " Production demand also exists on: "
                    + ", ".join(other_builds)
                )
            ...
            rows.append(
                {
                    ...
                    "required_this_build": fmt(
                        req["bom_remaining"]
                    ),
                    "allocated_this_build": fmt(
                        req["allocated"]
                    ),
                    "outstanding_this_build": fmt(
                        req["required"]
                    ),
                    "physical_buffer": fmt(
                        req["physical_buffer"]
                    ),
                    "planned_spillage": fmt(
                        req["spillage"]
                    ),
                    "on_order": fmt(
                        req["on_order"]
                    ),
                    "affected_builds": ", ".join(
                        req["builds"]
                    ),
                    "severity": risk.severity,
                    "risk": risk.label,
    ...
    def _rows_global(self):
        ...
        for req in calculated:
            ...
            row["planned_spillage"] += req[
                "spillage"
            ]
            row["shortage"] += req.get(
                "unfilled",
                Decimal("0"),
            )
            row["builds"].add(
        ...
        for row in grouped.values():
            ...
            if not self._should_show_global_requirement(
                row
            ):
                continue
        ...
        return [
            {
                ...
                "description": _(
                    "Physical stock buffer after satisfying "
                    "all Production Build Orders"
                ),
                "icon": "ti:alert-triangle:outline",
                "source": self.plugin_static_file(
                    "assembly_risk.js:renderPanel"
                ),
                "context": {
                    "mode": "build",
                    "rows": rows,
    ...
    def get_ui_dashboard_items(
        ...
        **kwargs,
        ...
        """Provide a global Assembly Risk exception report."""
        try:
        ...
        return [
            {
                ...
                "title": _(
                    "Assembly Risk - Production Build Orders"
                ),
                "description": _(
                    "Zero-buffer physical-stock risks across "
